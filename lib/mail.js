  ...
  <table cellpadding="0" cellspacing="0" border="0">
    <tr>
      <td style="vertical-align:middle; padding-right:11px;">
        <img src="${LOGO_URL}" alt="Naya Lumière" width="28" height="28"
             style="display:block; width:28px; height:28px; object-fit:contain;">
      </td>
      <td style="vertical-align:middle;">
        <div style="font-size:16px; font-weight:700; letter-spacing:0.06em; color:${C.text};
                    font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;
                    text-transform:uppercase; line-height:1;">NAYA LUMIÈRE</div>
        <div style="font-size:9px; letter-spacing:0.32em; color:${C.sub};
                    font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;
                    text-transform:uppercase; margin-top:3px; line-height:1;">COSMETICS</div>
      </td>
    </tr>
  </table>
`;
...
function productSuggestionBlock(products, sectionTitle) {
  ...
  const cards = products.map(p => {
    ...
    return `
      <td style="width:${cardWidth}px; vertical-align:top; padding:0 6px;">
        <a href="${url}" style="display:block; text-decoration:none; color:inherit;">
          <div style="background:${C.bg}; overflow:hidden;">
            <div style="background:${C.section}; width:100%; height:150px; display:flex; align-items:center; justify-content:center; overflow:hidden;">
              <img src="${img}" alt="${name}" width="${cardWidth}" height="150"
                   style="display:block; width:${cardWidth}px; height:150px; object-fit:contain; padding:12px; box-sizing:border-box;">
            </div>
            <div style="padding:10px 2px 0;">
              <p style="font-size:9px; font-weight:700; color:${C.muted}; margin:0 0 3px; text-transform:uppercase; letter-spacing:0.1em;">${p.brand_name}</p>
              <p style="font-size:12px; font-weight:600; color:${C.text}; margin:0 0 4px; line-height:1.3;">${name}</p>
              <p style="font-size:12px; font-weight:800; color:${C.purple}; margin:0;">${price}</p>
            </div>
          </div>
        </a>
      </td>`;
