...
export async function sendMarketingEmail(recipients, subject, htmlContent) {
  try {
    // Wrap the custom content in our brand template
    const html = emailWrapper(`
  ...
  try {
    const itemsHtml = cartItems.map(item => `
        <tr>
            <td>
                <div style="font-weight:700; color:${TEXT_DEEP};">${item.name}</div>
                <div style="font-size:11px; color:rgba(59,7,100,0.45); margin-top:2px;">${item.brand || 'Naya Lumière'}</div>
            </td>
            <td style="text-align:right; font-weight:700; color:${TEXT_DEEP};">AED ${Number(item.price).toFixed(2)}</td>
        </tr>
    `).join('');

    const content = `
        <h2>Something Beautiful Awaits</h2>
        <p>Dear ${firstName}, we noticed you left some of our botanical treasures in your cart. Your ritual is incomplete without them.</p>

        <div class="summary-box">
            <p style="margin:0; font-size:13px; color:${TEXT_MID}; font-weight:600; text-align:center;">Return to your cart today and bring home the science of radiance.</p>
        </div>
        ...
        <div style="text-align:center; margin-top:30px;">
            <a href="${process.env.NEXT_PUBLIC_BASE_URL || '#'}/cart" class="button">Complete Your Purchase</a>
        </div>
    `;
    ...
    await transporter.sendMail({
      from: `"Naya Lumière Cosmetics" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Wait! Your Naya Lumière treasures are still waiting...',
      html: emailWrapper(content, 'Abandoned Cart Notification'),
    });
    return { success: true };
  } catch (error) {
...
export async function sendTransactionNotificationEmail(recipients, txDetails) {
  try {
    ...
    // Fallback logic for Payment Method and Details
    ...
    const content = `
        ...
        <p>Detailed information for Stripe transaction <strong style="color:${TEXT_DEEP};">${id}</strong>.</p>

        <div class="summary-box">
            <div class="summary-item"><span>Status</span><span style="font-weight:700; color:${TEXT_DEEP}; text-transform:uppercase;">${status}</span></div>
            <div class="summary-item"><span>Type</span><span style="font-weight:700; color:${TEXT_DEEP}; text-transform:capitalize;">${type.replace(/_/g, ' ')}</span></div>
            <div class="summary-item"><span>Date</span><span style="font-weight:700; color:${TEXT_DEEP};">${new Date(created * 1000).toLocaleString()}</span></div>
        </div>

        <h3>Financial Breakdown</h3>
        <table style="margin-bottom: 24px;">
            <tr>
                <td style="font-weight: 800; color: rgba(59,7,100,0.65);">Gross Amount</td>
                <td style="text-align: right; font-weight: 800; color: ${TEXT_DEEP};">${(amount / 100).toFixed(2)} ${currency.toUpperCase()}</td>
            </tr>
            <tr>
                <td style="font-weight: 800; color: rgba(59,7,100,0.65);">Stripe Fee</td>
                <td style="text-align: right; font-weight: 800; color: #ef4444;">${(fee / 100).toFixed(2)} ${currency.toUpperCase()}</td>
            </tr>
            <tr>
                <td style="font-weight: 800; color: rgba(59,7,100,0.65); border-bottom: none;">Net Amount</td>
                <td style="text-align: right; font-weight: 800; color: #16a34a; border-bottom: none;">${(net / 100).toFixed(2)} ${currency.toUpperCase()}</td>
            </tr>
        </table>

        ${card ? `
        <h3>Payment Method</h3>
        <div style="background-color: #f8f0ff; border: 1px solid rgba(216,180,254,0.25); padding: 20px 24px; border-radius: 12px; margin-bottom: 24px;">
            <p style="margin-top: 0; margin-bottom: 12px;">
                <span style="font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em; color: ${LAV_DARK};">Card</span><br>
                <strong style="color: ${TEXT_DEEP}; font-size: 16px;">•••• •••• •••• ${card.last4 || 'N/A'}</strong><br>
                <span style="font-size: 13px; color: rgba(59,7,100,0.65);">Expires: ${card.exp_month || 'MM'}/${card.exp_year || 'YY'}</span>
            </p>
            <table style="margin: 0;">
                <tr><td style="padding: 4px 0; font-size: 13px; border: none; color: rgba(59,7,100,0.8);"><strong style="color: ${LAV_DARK}; font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; display: inline-block; width: 100px;">Brand/Issuer:</strong> <span style="text-transform: capitalize;">${card.brand || 'N/A'}</span></td></tr>
                <tr><td style="padding: 4px 0; font-size: 13px; border: none; color: rgba(59,7,100,0.8);"><strong style="color: ${LAV_DARK}; font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; display: inline-block; width: 100px;">Funding Type:</strong> <span style="text-transform: capitalize;">${card.funding || 'N/A'}</span></td></tr>
                <tr><td style="padding: 4px 0; font-size: 13px; border: none; color: rgba(59,7,100,0.8);"><strong style="color: ${LAV_DARK}; font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; display: inline-block; width: 100px;">Origin:</strong> ${card.country || 'N/A'}</td></tr>
                <tr><td style="padding: 4px 0; font-size: 13px; border: none; color: rgba(59,7,100,0.8);"><strong style="color: ${LAV_DARK}; font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; display: inline-block; width: 100px;">Fingerprint:</strong> <span style="font-family: monospace;">${card.fingerprint || 'N/A'}</span></td></tr>
            </table>
        </div>
        ` : ''}

        <h3>Cardholder Information</h3>
        <div class="address-box" style="margin-bottom: 24px;">
            <p style="margin-top: 0; margin-bottom: 12px; line-height: 1.4;">
                <strong style="color: ${TEXT_DEEP}; font-size: 15px;">${ownerName}</strong><br>
                <span style="color: ${LAV_DARK};">${ownerEmail || 'Email not provided'}</span>
            </p>
            ${billing?.address ? `
            <div style="font-size: 13px; color: rgba(59,7,100,0.7); border-top: 1px solid rgba(216,180,254,0.25); padding-top: 12px;">
                <strong style="color: ${LAV_DARK}; font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; display: block; margin-bottom: 4px;">Billing Address</strong>
                ...
                <strong style="text-transform: uppercase;">${billing.address.country || 'Address not provided'}</strong>
            </div>
            ` : ''}
        </div>

        <a href="${process.env.NEXT_PUBLIC_ADMIN_URL || '#'}/admin/payments" class="button">Go to Dashboard</a>
    `;

    await transporter.sendMail({
      from: `"Naya Lumière Cosmetics" <${process.env.EMAIL_USER}>`,
      to: recipients.join(', '),
      subject: `Transaction Details: ${id}`,
      html: emailWrapper(content, 'Transaction Notification'),
    });
    return { success: true };
  } catch (error) {
...
export async function sendPasswordResetEmail(email, firstName, resetUrl) {
  try {
    const content = `
        ...
        <p>Dear ${firstName}, a password reset has been requested for your Naya Lumière account.</p>
        ...
        <p style="font-size:13px; color:rgba(59,7,100,0.40); margin-top:24px;">If you did not request this, you can safely ignore this email. Your sanctuary remains protected.</p>
        <p style="font-size:11px; color:rgba(59,7,100,0.30); word-break:break-all; margin-top:12px;">Link: ${resetUrl}</p>
    `;

    await transporter.sendMail({
      from: `"Naya Lumière Cosmetics" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Password Reset - Naya Lumière',
