...
const brandHeader = `<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;width:auto;margin:0;"><tr><td width="1" style="vertical-align:middle;padding:0 8px 0 0;border-bottom:none;white-space:nowrap;"><img src="${LOGO_URL}" alt="Naya Lumière" width="48" height="48" style="display:block;width:48px;height:48px;object-fit:contain;"></td><td style="vertical-align:middle;padding:0;border-bottom:none;white-space:nowrap;"><div style="font-size:16px;font-weight:700;letter-spacing:0.06em;color:${C.text};font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;text-transform:uppercase;line-height:1;margin:0;padding:0;">NAYA LUMIÈRE</div><div style="font-size:9px;letter-spacing:0.32em;color:${C.sub};font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;text-transform:uppercase;margin:3px 0 0 0;padding:0;line-height:1;">COSMETICS</div></td></tr></table>`;
...
function paymentMethodBlock(paymentMethod, total_amount) {
  if (paymentMethod === 'tabby') {
    const installment = (Number(total_amount) / 4).toFixed(2);
    return `
      <h3>Payment</h3>
      <div style="background:#f0fdf9; border:1px solid #6ee7b7; padding:20px 24px; margin:24px 0;">
        <table cellpadding="0" cellspacing="0" border="0" width="100%">
          <tr>
            <td style="vertical-align:middle;">
              <div style="font-size:13px; font-weight:800; color:#065f46; text-transform:uppercase; letter-spacing:0.08em;">Tabby · Pay in 4</div>
              <div style="font-size:12px; color:#047857; margin-top:4px;">4 interest-free installments · No fees</div>
            </td>
            <td style="vertical-align:middle; text-align:right;">
              <div style="font-size:16px; font-weight:900; color:#065f46;">4 × AED ${installment}</div>
            </td>
          </tr>
        </table>
        <div style="margin-top:16px; border-top:1px solid #a7f3d0; padding-top:14px;">
          <table cellpadding="0" cellspacing="0" border="0" width="100%">
            <tr>
              <td style="font-size:11px; color:#047857; text-align:center; padding:4px 0;">Today<br><strong style="color:#065f46;">AED ${installment}</strong></td>
              <td style="font-size:18px; color:#a7f3d0; text-align:center;">·</td>
              <td style="font-size:11px; color:#047857; text-align:center; padding:4px 0;">1 month<br><strong style="color:#065f46;">AED ${installment}</strong></td>
              <td style="font-size:18px; color:#a7f3d0; text-align:center;">·</td>
              <td style="font-size:11px; color:#047857; text-align:center; padding:4px 0;">2 months<br><strong style="color:#065f46;">AED ${installment}</strong></td>
              <td style="font-size:18px; color:#a7f3d0; text-align:center;">·</td>
              <td style="font-size:11px; color:#047857; text-align:center; padding:4px 0;">3 months<br><strong style="color:#065f46;">AED ${installment}</strong></td>
            </tr>
          </table>
        </div>
      </div>`;
  ...
  if (paymentMethod === 'cashOnDelivery') {
    return `
      <h3>Payment</h3>
      <div class="box" style="font-size:14px; color:${C.sub};">Cash on delivery</div>`;
  }
  if (paymentMethod === 'card') {
    return `
      <h3>Payment</h3>
      <div class="box" style="font-size:14px; color:${C.sub};">Credit card</div>`;
  }
  return '';
}
...
export async function sendOrderConfirmationEmail(
  ...
  shippingAddress, giftWrapCost, couponCode = null, paymentMethod = null
) {
  try {
    ...
    const content = `
      ...
      ${paymentMethodBlock(paymentMethod, total_amount)}
...
export function buildStockClearanceEmail({ firstName, code, discountPercent, expiresAt, heroImage, products = [] }) {
  ...
  const topBar = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;margin:0;"><tr>
      <td align="center" bgcolor="${HP.promoStart}" style="background-color:${HP.promoStart};background-image:linear-gradient(90deg,${HP.promoStart} 0%,${HP.promoEnd} 100%);padding:10px 16px;border-bottom:none;font-family:${HP.font};font-size:12px;font-weight:600;color:#ffffff;line-height:1.6;">Use code <span style="display:inline-block;background-color:rgba(255,255,255,0.16);border:1px solid rgba(255,255,255,0.22);border-radius:999px;padding:1px 10px;font-weight:700;letter-spacing:0.04em;">${safeCode}</span> for ${discountPercent}% off &middot; Free delivery over AED 200</td>
    </tr></table>`;
  ...
  const trust = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;margin:36px 0 0;border-collapse:collapse;"><tr>
      ${['Free delivery over AED 200', 'Secure checkout', 'Official brand retailer'].map(text =>
