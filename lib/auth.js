    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          const result = await client.query('SELECT id, username, email, first_name, last_name, profile_image, password_hash, is_admin, created_at FROM users WHERE LOWER(email) = LOWER($1) ORDER BY id DESC LIMIT 1', [credentials.email]);
