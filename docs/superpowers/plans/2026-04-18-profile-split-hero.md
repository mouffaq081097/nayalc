# Profile Page — Split Hero Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the generic form-card profile page with a Split Hero layout — white glass left panel (avatar, tier, stats, mini nav) on desktop, horizontal identity bar on mobile.

**Architecture:** Single file rewrite of `app/account/profile/page.js`. The left panel is desktop-only (`hidden md:flex`); on mobile it collapses into a slim horizontal identity bar below the existing `AccountMobileTopBar`. Data for points/orders comes from the existing `useAccountData` hook already used by other account pages.

**Tech Stack:** Next.js 15 App Router, React hooks, Tailwind CSS 4, inline styles (Cloud Luxe tokens), Lucide icons, existing account context hooks.
...
|---|---|---|
| **Modify** | `app/account/profile/page.js` | Full rewrite — Split Hero layout, useAccountData, Discard button, fetched state |

No other files change. All imported components (`AccountMobileTopBar`, `useAccountData`, `setAccountNavDirection`) already exist.

---

## Task 1: Rewrite profile page — full implementation

**Files:**
- Modify: `app/account/profile/page.js`

- [ ] **Step 1: Replace the file contents**

Replace the entire contents of `app/account/profile/page.js` with:
...
Navigate to `https://localhost:3000/account/profile` (logged in).

**Desktop checklist:**
- [ ] Left panel is visible, white glass, `position: sticky`
- [ ] Avatar shows correct initial (first letter of first name)
- [ ] Tier label and badge pull from `loyaltyData.stats.tier` (e.g. "Silver Member")
- [ ] Points and Orders count show real numbers
- [ ] "Profile" nav item is highlighted in lavender
- [ ] Clicking "Orders" in mini nav navigates to `/account/orders`
- [ ] Form fields are in two groups: "Personal info" and "Contact"
- [ ] Email field is read-only (cursor: not-allowed, dimmed)

**Mobile checklist (resize browser to < 768px):**
- [ ] Left panel is hidden
- [ ] `AccountMobileTopBar` shows "My Profile" + back arrow
- [ ] Identity bar shows avatar circle, name, tier, points
- [ ] Form shows below identity bar, full width
- [ ] Save button is full-row on mobile

**Interaction checklist:**
- [ ] Edit first name → save → name updates in left panel on next load
- [ ] Edit first name → click Discard → field resets to saved value
- [ ] Clear first name → submit → error "First name and last name are required." appears
- [ ] Save succeeds → button shows "✓ Saved" for ~2.5s then returns to "Save changes"

- [ ] **Step 3: Commit**

```bash
git add app/account/profile/page.js
git commit -m "feat: redesign profile page with split hero layout"
```
...
**Spec coverage:**
- ✅ Desktop split hero with white left panel
- ✅ Avatar (initials), name, email, tier badge, divider, stats (points + orders), brand quote, mini nav
- ✅ Mini nav uses `setAccountNavDirection` + `router.push` — matches AccountMenuList pattern
- ✅ Mobile: AccountMobileTopBar + horizontal identity bar + form
- ✅ Personal info group (first + last, 2-col) + Contact group (email read-only, phone)
- ✅ Save + Discard buttons; Discard resets to `fetched` state
- ✅ No conflict with AccountMenuList (`md:hidden`, different page)
- ✅ All style tokens match spec exactly

**Placeholder scan:** None — all code blocks are complete.

**Type consistency:** `loyaltyData.stats.tier`, `loyaltyData.stats.points`, `orders.length` — consistent across all references. `fetched` state mirrors `form` shape exactly.
