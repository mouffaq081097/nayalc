                ...
                >
                    ...
                    >
                        <Image 
                            ...
                            alt={product.imagesData?.[selectedImage]?.alt || `${product.name} - Premium Skincare at nayalc.com`}
                            width={800} 
                            height={1000} 
                            priority
                    ...
                    <div className="hidden lg:flex justify-center gap-4">
                        {product.images.map((img, index) => (
                            <button key={index} onClick={() => setSelectedImage(index)} className={`relative w-12 h-12 rounded-full overflow-hidden border-2 transition-all p-1 bg-[#f5f5f7] ${selectedImage === index ? 'border-gray-900 scale-105 shadow-md' : 'border-transparent opacity-40 hover:opacity-100'}`}>
                                <Image src={img} alt={product.imagesData?.[index]?.alt || `${product.name} - Detail ${index + 1}`} width={48} height={48} className="w-full h-full object-contain mix-blend-multiply" />
                            </button>
                        ))}
                    </div>
                )}
